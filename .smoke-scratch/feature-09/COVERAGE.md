...
| ST2 | `match="text"`: substring against `Feature.description` **OR** `Feature.scenario.name` (either field; tech-04); at most one hit per file; `matched_field` stays `"description"` for both. | Public surface → Storage | `F09_04_match_text.py` (+ cross-credit `F02_07` SR1) | covered |
...
| MS5 | `match="text"` matches anywhere in `Feature.description` (real-newline string, not the on-disk literal `\n`) and in `Feature.scenario.name` (tech-04). | Invariants → Match semantics | `F09_04_match_text.py` (multi-line description + a scenario-name-only hit) | covered |
