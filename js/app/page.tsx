...
function Workspace() {
  ...
  async function deleteCheckedFeatures() {
    const paths = [...checkedFeatures];
    if (!paths.length || !(await confirm({ title: "Delete selected scenarios?", description: `Delete ${paths.length} selected scenario${paths.length === 1 ? "" : "s"} from this folder?`, confirmLabel: "Delete", destructive: true }))) return;
    try {
      for (const path of paths) await api(pathUrl("/api/files", path), { method: "DELETE" });
      await refreshTree();
      await openFolder(selectedPath);
    } catch (cause) { setError((cause as Error).message); }
  }
